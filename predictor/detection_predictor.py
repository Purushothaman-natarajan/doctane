...
from doctane.model.detection.utils import _remove_padding
...
class DetectionPredictor(nn.Module):
    ...
    def forward(
        ...
        **kwargs: Any,
        ...
        if return_maps:
            seg_maps = [
                pred.permute(1, 2, 0).detach().cpu().numpy() for batch in predicted_batches for pred in batch["out_map"] #.unbind(0)
            ]
