...
def main(rank: int, world_size: int, args):
    ...
    Args:
        rank (int): Process rank (used for selecting the device and coordinating DDP).
    ...
    # Load validation dataset on rank 0 (single process to save memory)
    if rank == 0:
        st = time.time()
    ...
    model = DDP(model, device_ids=[device])  # Wrap model with DistributedDataParallel
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="Distributed training script for text detection models",
        formatter_class=argparse.ArgumentDefaultsHelpFormatter,
    )

    # Distributed Data Parallel (DDP) settings
    parser.add_argument("--backend", default="nccl", type=str,
                        help="DDP backend to use (e.g., nccl for GPUs, gloo for CPUs)")

    # Required model and dataset paths
    parser.add_argument("arch", type=str,
                        help="Model architecture name to train (e.g., 'dbnet', 'Linknet')")
...
if __name__ == "__main__":
    ...
    if not torch.cuda.is_available():
        raise AssertionError("PyTorch cannot access your GPUs. Please ensure CUDA is available.")

    # Default to using all available GPUs if not specified
    if not isinstance(args.devices, list) or args.devices is None:
        args.devices = list(range(torch.cuda.device_count()))

    nprocs = len(args.devices)  # Number of processes to spawn (1 per GPU)

    # Required environment variables for torch.distributed.init_process_group
    os.environ["MASTER_ADDR"] = "localhost"
    os.environ["MASTER_PORT"] = "29500"

    # Start multi-processing training using spawn (safe for DDP initialization)
