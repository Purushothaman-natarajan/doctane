...
from doctane.utils.common_types import BoundingBox
from doctane.utils.geometry import resolve_enclosing_bbox, resolve_enclosing_rbbox
from doctane.utils.reconstitution import synthesize_kie_page, synthesize_page
from doctane.utils.repr import NestedObject
from doctane.utils.visualization import visualize_kie_page, visualize_page
...
class Page(Element):
    ...
    def show(self, interactive: bool = True, preserve_aspect_ratio: bool = False, **kwargs) -> None:
        ...
        """
        visualize_page(self.export(), self.page, interactive=interactive, preserve_aspect_ratio=preserve_aspect_ratio, add_labels=True)
        plt.show(**kwargs)
    ...
    def export_as_xml(self, file_title: str = "doctane - XML export (hOCR)") -> tuple[bytes, ET.ElementTree]:
        ...
        SubElement(
            head,
            "meta",
            attrib={"name": "ocr-system", "content": f"python-doctan {doctane.__version__}"},  # type: ignore[attr-defined]
        )
        SubElement(
            head,
            "meta",
...
class KIEPage(Element):
    ...
    def export_as_xml(self, file_title: str = "doctane - XML export (hOCR)") -> tuple[bytes, ET.ElementTree]:
        ...
        SubElement(
            head,
            "meta",
            attrib={"name": "ocr-system", "content": f"python-doctane {doctane.__version__}"},  # type: ignore[attr-defined]
        )
        SubElement(
            head,
            "meta",
