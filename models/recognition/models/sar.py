...
from doctane.models.recognition.recog_utils import RecognitionModel, RecognitionPostProcessor

__all__ = ["SAR", "sar_resnet18", "sar_resnet34", "sar_resnet50", "sar_resnet101", "sar_resnet152"]
...
class SARDecoder(nn.Module):
    ...
    def __init__(
        ...
        self.rnn_units = rnn_units  # Store the rnn_units
    ...
    def forward(
        ...
        for t in range(self.max_length + 1):  # 32
            if t == 0:
                # Initialize hidden and cell states with rnn_units (instead of features.size(1))
                hidden_state_init = cell_state_init = torch.zeros(
                    features.size(0), self.rnn_units,  # Use self.rnn_units here
                    device=features.device, dtype=features.dtype
                )
                hidden_state, cell_state = hidden_state_init, cell_state_init
                prev_symbol = holistic
            elif t == 1:
                # Initialize with a 'blank' sequence of length vocab_size + 1 filled with zeros
...
def sar_resnet18(pretrained: bool = False, **kwargs: Any) -> SAR:
    """SAR with a ResNet-18 feature extractor."""
    return _sar(
        "sar_resnet18",
        pretrained,
        resnet18,
...
def sar_resnet34(pretrained: bool = False, **kwargs: Any) -> SAR:
    """SAR with a ResNet-34 feature extractor."""
    return _sar(
        "sar_resnet34",
        pretrained,
        resnet34,
...
def sar_resnet50(pretrained: bool = False, **kwargs: Any) -> SAR:
    """SAR with a ResNet-50 feature extractor."""
    return _sar(
        "sar_resnet50",
        pretrained,
        resnet50,
...
def sar_resnet101(pretrained: bool = False, **kwargs: Any) -> SAR:
    """SAR with a ResNet-101 feature extractor."""
    return _sar(
        "sar_resnet101",
        pretrained,
        resnet101,
...
def sar_resnet152(pretrained: bool = False, **kwargs: Any) -> SAR:
    """SAR with a ResNet-152 feature extractor."""
    return _sar(
        "sar_resnet152",
        pretrained,
        resnet152,
